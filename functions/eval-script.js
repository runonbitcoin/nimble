...
const verifyTxSignature = require('./verify-tx-signature')
const verifyTxSignatureAsync = require('./verify-tx-signature-async')
const ripemd160Async = require('./ripemd160-async')
const sha1Async = require('./sha1-async')
const sha256Async = require('./sha256-async')
const decodePublicKey = require('./decode-public-key')
...
function evalScript(
  ...
  function finish(error = null) {
    ...
    const success =
      !error && !!stack.length && stack[stack.length - 1].some((x) => x)
    ...
    function step() {
      ...
      if (branchExec.length > 0 && !branchExec[branchExec.length - 1]) {
        ...
        while (i < chunks.length) {
          ...
          const executed =
            (prevOp === OP_IF && sub === 0) ||
            ([OP_ELSE, OP_ENDIF].includes(prevOp) && psub === 0)
      ...
      switch (chunk.opcode) {
        ...
        case OP_EQUAL:
          {
            ...
            const b = pop()
            const equal =
              a.length === b.length && !a.some((ai, i) => ai !== b[i])
        ...
        case OP_1ADD:
          stack.push(
            encodeNum(addNum(decodeNum(pop()), { num: BigInt(1), neg: false }))
          )
          break
        case OP_1SUB:
          stack.push(
            encodeNum(addNum(decodeNum(pop()), { num: BigInt(1), neg: true }))
          )
          break
        case OP_NEGATE:
          {
        ...
        case OP_GREATERTHAN:
          stack.push(
            encodeNum(greaterThan(decodeNum(pop()), decodeNum(pop())) ? 1 : 0)
          )
          break
