...
function sighash (tx, vin, parentScript, parentSatoshis, sighashFlags, async) {
  ...
  function getSighash (hashPrevouts, hashSequence, hashOutputs) {
    const writer = new BufferWriter()
  ...
  if (async) {
    return Promise.all([hashPrevouts, hashSequence, hashOutputs]).then(args => getSighash(...args))
  } else {
    return getSighash(hashPrevouts, hashSequence, hashOutputs)
  }
}
