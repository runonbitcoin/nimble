...
const { generatePrivateKey, calculatePublicKey, ecdsaSign, sha256 } =
  nimble.functions
  ...
  it('valid', () => {
    for (let i = 0; i < 10; i++) {
      ...
      const bsvPrivateKey = new bsv.PrivateKey(
        bsv.crypto.BN.fromBuffer(bsv.deps.Buffer.from(privateKey))
      )
      ...
      const sig = ecdsa
        .set({ hashbuf, endian, privkey: bsvPrivateKey })
        .sign().sig

      expect(Buffer.from(signature.r).toString('hex')).to.equal(
        new bsv.crypto.BN(bsv.deps.Buffer.from(sig.r.toArray()))
          .toBuffer()
          .toString('hex')
      )
      expect(Buffer.from(signature.s).toString('hex')).to.equal(
        new bsv.crypto.BN(bsv.deps.Buffer.from(sig.s.toArray()))
          .toBuffer()
          .toString('hex')
      )
