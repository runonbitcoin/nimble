...
describe('readVarint', () => {
  it('valid', () => {
    ...
    expect(
      readVarint(new BufferReader([0xfe, 0x00, 0x00, 0x00, 0x00]))
    ).to.equal(0)
    expect(
      readVarint(new BufferReader([0xfe, 0xfc, 0xfd, 0xfe, 0xff]))
    ).to.equal(0xfffefdfc)
    expect(
      readVarint(
        new BufferReader([0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
      )
    ).to.equal(0)
    expect(
      readVarint(
        new BufferReader([0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x1f, 0x00])
      )
  ...
  it('multiple times', () => {
    const reader = new BufferReader([
      0x00, 0x01, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x1f, 0x00,
    ])
  ...
  it('throws if too big', () => {
    expect(() =>
      readVarint(
        new BufferReader([0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x20, 0x00])
      )
    ).to.throw('varint too large')
    expect(() =>
      readVarint(
        new BufferReader([0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff])
      )
    ).to.throw('varint too large')
  })
  ...
  it('throws if not enough data', () => {
    ...
    expect(() =>
      readVarint(new BufferReader([0xfe, 0x00, 0x00, 0x00]))
    ).to.throw(err)
    expect(() => readVarint(new BufferReader([0xff, 0x00]))).to.throw(err)
    expect(() =>
      readVarint(
        new BufferReader([0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
      )
    ).to.throw(err)
    expect(() => readVarint(new BufferReader([0xff, 0x00]))).to.throw(err)
  })
