...
describe('writeU64LE', () => {
  it('valid', () => {
    expect(
      Array.from(writeU64LE(new BufferWriter(), 0).toBuffer())
    ).to.deep.equal([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
    expect(
      Array.from(
        writeU64LE(new BufferWriter(), Number.MAX_SAFE_INTEGER).toBuffer()
      )
  ...
  it('throws if too big', () => {
    expect(() =>
      writeU64LE(new BufferWriter(), Number.MAX_SAFE_INTEGER + 1)
