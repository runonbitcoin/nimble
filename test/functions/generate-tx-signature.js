...
describe('generateTxSignature', () => {
  it('generates signature that bsv library validates', async () => {
    for (let i = 0; i < 10; i++) {
      ...
      const parentScript = createP2PKHLockScript(
      ...
      const scriptSig = new bsv.Script(
        encodeHex(createP2PKHUnlockScript(txsignature, publicKey.toBuffer()))
      )
      ...
      const scriptSig2 = new bsv.Script(
        encodeHex(createP2PKHUnlockScript(txsignature2, publicKey.toBuffer()))
      )
      ...
      const verified2 = interpreter2.verify(
        scriptSig2,
        scriptPubkey2,
        bsvtx2,
        vin,
