...
describe('writeVarint', () => {
  it('valid', () => {
    const testValid = (n, enc) =>
      expect(
        Array.from(writeVarint(new BufferWriter(), n).toBuffer())
      ).to.deep.equal(enc)
    testValid(0, [0])
    testValid(1, [1])
    testValid(252, [252])
    ...
    testValid(
      0x0000f2f3f4f5f6f7,
      [0xff, 0xf7, 0xf6, 0xf5, 0xf4, 0xf3, 0xf2, 0x00, 0x00]
    )
    testValid(
  ...
  it('throws if too big', () => {
    expect(() =>
      writeVarint(new BufferWriter(), Number.MAX_SAFE_INTEGER + 1)
    ).to.throw('varint too large')
    expect(() => writeVarint(new BufferWriter(), Number.MAX_VALUE)).to.throw(
      'varint too large'
    )
  })
})
